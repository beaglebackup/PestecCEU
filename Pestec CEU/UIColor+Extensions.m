//
//  UIColor+Extensions.m
//  JackOfTrades
//
//  Created by Admin on 8/16/14.
//  Copyright (c) 2014 Gabriel. All rights reserved.
//

#import "UIColor+Extensions.h"

@implementation UIColor (Extensions)


+ (UIColor *)colorLavender {
    return [UIColor colorWithRed:188.0/255.0 green:147.0/255.0 blue:193.0/255.0 alpha:1];
}

+ (UIColor *)colorLightBeige {
    return [UIColor colorWithRed:238.0/255.0 green:234.0/255.0 blue:230.0/255.0 alpha:1];
}


+ (UIColor *)colorPurple {
    return [UIColor colorWithRed:117/255.0 green:40/255.0 blue:220/255.0 alpha:1];
}


+ (UIColor *)colorOffWhite {
    return [UIColor colorWithRed:248/255.0 green:248/255.0 blue:248/255.0 alpha:1];
}

+ (UIColor *)colorGray {
    return [UIColor colorWithRed:165/255.0 green:165/255.0 blue:165/255.0 alpha:1];
}

+ (UIColor *)colorMarineBlue {
    return [UIColor colorWithRed:51/255.0 green:69/255.0 blue:79/255.0 alpha:1];
}

+ (UIColor *)colorLimeGreen {
    return [UIColor colorWithRed:187/255.0 green:212/255.0 blue:59/255.0 alpha:1];
}

+ (UIColor *)colorOffBlack {
    return [UIColor colorWithRed:12/255.0 green:0/255.0 blue:0/255.0 alpha:1];
}

+ (UIColor *)colorBrightBlue {
    return [UIColor colorWithRed:0/255.0 green:121/255.0 blue:254/255.0 alpha:1];
}




@end
